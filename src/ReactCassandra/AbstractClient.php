...
abstract class AbstractClient extends \Evenement\EventEmitter
{
    ...
    {
        $frame = new \ReactCassandra\Protocol\StartupFrame();
        $frame->fromParams();
    ...
    public function sendFrame(\ReactCassandra\Protocol\AbstractFrame $frame)
    {
    ...
    {
        $frame = new \ReactCassandra\Protocol\QueryFrame();
        $frame->fromParams([
            'cql' => $cql,
    ...
    public function onData($readData = null)
    {
        ...
        do {
            $frame = null;
            $bytesParsed = \ReactCassandra\Protocol\AbstractFrame::parseBuffer($this->buffer, $frame);
            if (!is_null($frame)) {
                $this->onFrame($frame);
            }
            if ($bytesParsed > 0) {
    ...
    public function onFrame(\ReactCassandra\Protocol\AbstractFrame $frame)
    {
        switch (true) {
            case $frame instanceof \ReactCassandra\Protocol\ErrorFrame:
                ...
                break;
            case $frame instanceof \ReactCassandra\Protocol\ReadyFrame:
                $this->status = Constants::CLIENT_CONNECTED;
